...
4. Use os agentes diretamente no KiloCode
    ...
    private async fetchAvailableAgents(): Promise<any[]> {
        try {
            const config = vscode.workspace.getConfiguration('zanaiCode');
            const serverUrl = config.get('serverUrl', 'http://localhost:3000');
            const response = await axios.get(`${serverUrl}/api/agents`);
    ...
    private async downloadAndInstallAgent(agent: any): Promise<void> {
        ...
        const config = vscode.workspace.getConfiguration('zanaiCode');
        const serverUrl = config.get('serverUrl', 'http://localhost:3000');
        const response = await axios.get(`${serverUrl}/api/agents/${agent.id}/export`);
        ...
        existingModes.customModes = existingModes.customModes.filter(
            (mode: any) => mode.slug !== agentMode.slug
        );

        existingModes.customModes.push(agentMode as any);
        ...
        if (agentConfig.rules && agentConfig.rules.length > 0) {
            ...
            // Write rule files
            agentConfig.rules.forEach((rule: any) => {
